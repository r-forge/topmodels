...
set.seed(1090)
y1 <- rnbinom(100, mu = 3, size = 2)
y2 <- rpois(100, lambda = 3)

curve(dnbinom(x, mu = 3, size = 2), from = -0.05, to = 0.05)
curve(dpois(x, lambda = 3), from = -0.05, to = 0.05, add = TRUE, col = 2)
legend("topleft", c("nbinom", "pois"), lty = 1, col = c(1, 2))

## correctly specified Poisson model fit (mu = 3.34)
m1_corr <- glm(y2 ~ 1, family = poisson)
     
## misspecified Poisson model fit (mu = 3.32):
## wrong assumpion of underlying distribution -> underdispersive model fit (residuals)
## [U-shaped PIT, S-shaped QQ-plot]
m1_missp <- glm(y1 ~ 1, family = poisson)
...
legend("topleft", c("t_4", "norm"), lty = 1, col = c(1, 2))
...
legend("topleft", c("norm", "unif", "laplace"), lty = 1, col = c(1, 2, 3))
...
legend("topleft", c("norm", "weibull_1.5", "weibull_12"), lty = 1, col = c(1, 2, 3))
...
# -------------------------------------------------------------------
# (5) LEAVING OUT RELEVANT REGRESSORS: MISSPECIFIED MEAN AND VARIANCE
# -------------------------------------------------------------------
ens_mean <- rnorm(5000, 0.35, 6.91)
ens_logsd <- rnorm(5000, -0.56, 0.43)
