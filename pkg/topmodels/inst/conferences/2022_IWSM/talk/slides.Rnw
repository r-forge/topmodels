...
@
    ...
    \begin{itemize}
        \item Daily records over 13 years ($2000-2013$), Innsbruck Airport (AUT)
        \item Observations precipitation amount $mm/3\,days$ (`\code{Rain}')
        \visible<2>{\item Accumulated precipitation from 11-member ensemble (`\code{ensmean}')}
        %%\item Station Innsbruck Airport, Austria.
    \end{itemize}

<<data_figs, echo = FALSE, results = hide>>=
pdf(file = "slides_data_figs_hist.pdf", width = 5, height = 4)
hist(RainIbk$rain,
     main = NA,
     xlab = "Rain",
     ylab = "Density",
...
pdf(file = "slides_data_figs_scatter.pdf", width = 5, height = 4)
...
%%        Importance of probabilistic forecasts
%%    \end{block}
%%
...
%%    \begin{itemize}
%%        \item Improve accuracy and reliability of raw forecasts.
%%        \item Account for both, errors in expectation but also uncertainty.
%%    \end{itemize}
%%
%%    \vspace{1em}
%%    \textbf{Many models available}
%%    \begin{itemize}
%%        \item Generalized linear models (GLMs),
%%        \item generalized additive models (GAMs),
%%        \item generalized linear models for location, scale, and shape (GAMLSS),
%%        \item and many more.
%%    \end{itemize}
%%
%%    \vspace{1em}
%%    $\BoldRightarrow$ We revisit the models by Messner, Mayr, and Zeileis (2010)
%%        to introduce graphical assessment techniques.
%%
%%\end{frame}
