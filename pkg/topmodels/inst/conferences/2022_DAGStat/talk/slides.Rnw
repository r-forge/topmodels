...
\item Visualizations especially suitable for evaluating potential lack of fits.
\end{itemize}
...
%-------------------------------------------------------------------
\subsection{Probablities of goals in the 2018 FIFA World Cup}
%-------------------------------------------------------------------
...
\textbf{Covariates:}
\begin{itemize}
\item Difference in estimated log-ability of the competing teams.
\end{itemize}
...
\item[] $y_i \sim \, Pr\big(\log(\lambda_i = \beta_0 + \beta_1 \cdot \text{logability}_i)\big)$.
\end{itemize}
...
idx <- c(121, 122)
df <- data.frame(group =  FIFA2018$team[idx], lambda = fitted(m)[idx], label = sprintf("lambda = %s", round(fitted(m)[idx], 2)))
ggplot(df, aes(y = group, xdist = dist_poisson(lambda))) + 
  stat_halfeye() +  
  geom_text(aes(x = c(8, 8), y = c(2.1, 3.2) - 0.2, label = label)) +
...
rootogram(m, style = "standing", scale = "raw", fitted = TRUE, xlab = "Goals")
@
...
\item[] $ y_i \sim \, \beta_{0} + \beta_1 \cdot \text{ensmean}_i$
\item Heteroscedastic censored regression with a logistic distribution assumption:
\item[] $\text{y}_i \sim \, Log\big(\mu_i = \beta_{0} + \beta_1 \cdot \text{ensmean}_i, \, \log(\sigma_i = \gamma_0 + \gamma_1 \cdot \text{enssd}_i)\big)$
\end{itemize}
...
\begin{center}
\setkeys{Gin}{width=0.65\textwidth}
<<rain_scatter1, fig=TRUE, echo=FALSE, height=5.6, width=7.8>>=
...
\begin{center}
\setkeys{Gin}{width=0.65\textwidth}
<<rain_scatter2, fig=TRUE, echo=FALSE, height=5.6, width=7.8>>=
plot(rain ~ ensmean, data = RainIbk, pch = 19, col = gray(0, alpha = 0.2))
...
ggplot2::autoplot(c(p1, p2), colour = c(3, 4), style = "line", single_graph = TRUE, confint_fill = 1, confint = "polygon", ref = FALSE)
@
\end{center}
\end{frame}
