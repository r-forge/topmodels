...
\begin{center}
\setkeys{Gin}{width=0.65\textwidth}
<<rain_topmodels_lm_pithist1, fig=TRUE, echo=FALSE, height=5., width=8.8>>=
...
<<echo=TRUE,eval=FALSE>>=
rootogram(forest, plot = FALSE, breaks = -9:14) |>
  autoplot(legend = TRUE, col = 1)
@

\begin{center}
\setkeys{Gin}{width=0.65\textwidth}
<<rain_topmodels_lm_rootogram1, fig=TRUE, echo=FALSE, height=5., width=8.8>>=
autoplot(c(forest = root1b_df1), legend = TRUE, col = 1, ylim = c(-9, 19))
@
\end{center}
...
autoplot(c(linear = root1b_lm1), legend = TRUE, ylim = c(-9, 19))
@
\end{center}
