...
$y_i$ the PIT $u_i$ can be generated as a random draw from the interval
$[\mathbf{\mathcal{F}}(y_i - 1 | \, \hat{\boldsymbol{\theta}}_i),
\mathbf{\mathcal{F}}(y_i | \, \hat{\boldsymbol{\theta}}_i)]$. Even if this
leads to some randomness in the graphical representation of PIT values, for
cases with a high number of observations the impact on the graphical evaluation
when repeating the calculations (i.e. drawing new values $u_i$) is typically
rather small. For small data sets, we recommend to increase the number of random draws
which significantly reduces the randomness in the graphical display.
...
PIT values $u_i$ the expected fraction of the CDF along the interval
$[\mathbf{\mathcal{F}}(y_i - 1 | \, \hat{\boldsymbol{\theta}}_i),
\mathbf{\mathcal{F}}(y_i | \, \hat{\boldsymbol{\theta}}_i)]$ is used. (FIXME: Probably wrong!) In more detail,
calibration is assessed by aggregating over the observtions $y_i$  and comparing the mean 
PIT,


$$\bar{u}_i = \bar{\mathbf{\mathcal{F}}}(y_i | \, \hat{\boldsymbol{\theta}}_i)$$
