...
parametric univariate distributional model, with
estimated parameters $\hat{\boldsymbol{\theta}}_{i} = (\hat{\theta}_{i1},
\ldots, \hat{\theta}_{iK})^\top$ and $\mathcal{f}( \cdot )$ desribing the
...
Given an observational vector of a discrete random variable $y_i (i = 1, \ldots,
n)$, rootograms check whether observed frequencies $\text{obs}_j$ match the
expected frequencies $\text{exp}_j$; hence, the observed and expected frequency
are given for each integer $j$ by
...
distributional model $\mathcal{f}( \cdot )$ and $w_i being
