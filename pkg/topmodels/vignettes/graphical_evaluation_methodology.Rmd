...
[FIXME: Rewrite whole section "rootogram", just tried to transfer terminology of other
methods to rootograms.] To assess the goodness of fit of a modeled distribution
$\mathcal{D}( \cdot )$ with estimated parameters $\hat{\boldsymbol{\theta}}_{i}
= (\hat{\theta}_{i1}, \ldots, \hat{\theta}_{iK})^\top$, @Kleiber+Zeileis:2016
proposes so called rootograms.  Rootograms check whether observed frequencies
$\text{obs}_j$ match the expected frequencies $\text{exp}_j$; for a discrete
random variable $y_i$, the observed and expected frequency for each integer $j$
are given by

$$\text{obs}_j = \sum_{i=1}^{n}I(y_i - j),$$
$$\text{exp}_j = \sum_{i=1}^{n}\mathcal{D}(j | \hat{\boldsymbol{\theta}}_{i}),$$

where $I( \cdot )$ is an indicator variable and $\mathcal{D}( \cdot )$ the
density function (probability mass function).

In case of a discrete random variable $y_i$, one can use a set of breaks $b_0,
b_1, b_2, \dots~$ that span a suitable subset of $y_i$. Here, the observed and
expected frequencies are given by

$$\text{obs}_j = \sum_{i=1}^{n}w_{i} I(y_i \in (b_j, b_{j+1}]),$$
$$\text{exp}_j = \sum_{i=1}^{n}w{\mathbf{\mathcal{F}}(b_{j+1} | \hat{\boldsymbol{\theta}}_{i}) - \mathbf{\mathcal{F}}(b_{j} | \hat{\boldsymbol{\theta}}_{i})},$$

with $\mathbf{\mathcal{F}}( \cdot )$ being the CDF of the modeled distribution
and $w_i (i= 1, \dots, n)$ being optional observation-specific weights.
According to @Kleiber+Zeileis:2016, the weights are needed for survey data and
also for situations with model-based weights.
...
standard normal distribution $\Phi^{-1}$ evaluated at the values $u_i$, hence,
they can be defined as 

$$\hat{r}_i = \Phi^{-1}(\mathbf{\mathcal{F}}(y_i | \, \hat{\boldsymbol{\theta}}_{i})) = u_i,$$ 
...
standard normally distributed. Hence, to check for normality, quantile residuals
can be graphically compared to theoretical quantiles of the standard normal distribution,
 where strong deviations from the
bisecting line indicate a misspecified model fit.
This graphical evaluation is well known as normal probability plot or normal Q-Q plot [@Hoaglin:2006].

But Q-Q plots can also be applied to check if residuals follow any other known 
distribution, by employing any inverse cumulative distribution function of interest in the 
computation of quantile residuals and comparing these to the respective theoretical
quantiles. This is called than a theoretical quantile-quantile plot or Q-Q plot for short [@Friendly:1991].
...
de-trended Q-Q plot [@Friendly:1991] is best known due to the application in
@Buuren+Fredriks:2001, and therefore mostly referred to their introduced version
called worm plot. 

## Summary plot
```{r plot-qq, echo = FALSE, results = "hide"}
grDevices::svg(file = "figures/graph_eval_meth_qq.svg", width = 14, height = 16)
par(mfrow = c(7, 7), bg = "#f7f7f7")
