...
probability distribution [GAMLSS paper, Klein et al, Hothorn/Kneib/Buehlmann].
In the simplest case this could be a generalized linear model
[GLM, McCullagh & Nelder 1989] where higher moments of the probability distribution
co-vary with the expectation. Instead, multiple distribution parameters could
also be linked to explanatory variables as in distributional regression [Klein at al.]
or generalized additive models of location, scale, and shape [GAMLSS ref].
Moreover, many other flexible modeling techniques yield fitted distributions,
including transformation models [Hothorn], Bayesian modeling [Umlauf et al.],
machine learning approaches [refs], and many others.

For assessing the goodness of fit of such probabilistic regression models, (proper)
scoring rules [refs] are widely used, e.g., using the log-likelihood (also
known as the log-score) or the continuous ranked probability score [CRPS, ref].
...
been established. Here, we will provide such a unified framework that facilitates:

- adoption of these graphics for a broad range of models/distributions,
- understanding what the graphics have in common and what sets them apart,
- demonstration of relative strengths and weaknesses in uncovering sources of lack of fit,
- fine-tuning the graphics for certain models or data sets.


_FIXME (include shortened version of Moritz' text):_ Before discussing the various graphical evaluation methods in more detail, we
want to outline the underlying motivation and identify similarities and
differences in the methodologies. According to the seminal work of
...
_FIXME:_ Look ahead. Discuss the two basic approaches. Either marginal or conditional.
