...
models. The unifying prediction method provides users with many different
...
To be able to use the object-oriented framework of _topmodels_, solely a
`procast()` method must exist for the model class of interest. Currently the
package provides generic `procast` methods the model classes `lm`, `glm`,
`crch`, and `disttree`.
