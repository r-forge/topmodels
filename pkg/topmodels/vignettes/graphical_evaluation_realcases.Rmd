...
This yields the same predicted Poisson distribution for all teams and matches
in this tournament - a slightly unrealistic assumption:

```{r}
p_ic <- Poisson(lambda = fitted(m_ic))
head(p_ic)
```
...
Now, the slope of $`r round(coef(m_reg)[2], digits = 3)`$ can be interpreted
...
In comparison to the intercept-only models, this 
yields a different predicted Poisson distribution for each team/match
...
```{r}
p_reg <- Poisson(lambda = fitted(m_reg))
head(p_reg)
```
...
scale. This makes it expecially useful for evaluating count data models which predictions are 
often close to zero. 

To do so, we compute the expected absolute frequencies by averaging across the
expectations per team/match from the regression model and compare these
for both estimated models with the observed frequencies:

```{r}
observed <- table(FIFA2018$goals)
expected_ic <- pdf(p_ic, 0:6)
expected_ic <- colMeans(expected_ic) * sum(observed)
expected_reg <- pdf(p_reg, 0:6)
expected_reg <- colMeans(expected_reg) * sum(observed)
cbind(observed, expected_ic, expected_reg) 
```

Graphically, this comparison is a so-called _rootogram_
...
`rootogram()`, we can easily reproduce the previous results just using the fitted model objects 
`m_ic` and `m_reg`:
...
Here, the offset around the x-axis compared to a reference line clearly shows the difference between the two frequencies for the intercept only model and the full probabilistic model. For both models the differences are reasonably close to zero indicating a rather good marginal fit:
