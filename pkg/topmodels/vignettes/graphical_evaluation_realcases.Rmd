...
point mass at zero. Additionally, a wavelike pattern indicates a slight overfitting of precipitation 
sums between zero and 5mm. In contrast, the censored logistic regression `m_hclog`
...
For the linear model, the underfitting at the lower tail
is not visible in the lowest interval but only in the subsequent intervals,
indicating that more values occur here than expected (FIXME: (ML) Is this
correct?!):
...
FIXME: (ML) Compare pattern "too skew to the left" in the summary!

```{r}
pithist(m_lm, breaks = 50)
pithist(m_lm, breaks = 50, trafo = qnorm, xlim = c(-3, 3))
```

For a better comparison of the two model fits, alternatively a "line" `style`
_PIT histogram_ can be shown:

```{r}
ggplot2::autoplot(c(p1, p2), colour = c(2, 4), style = "line", single_graph = TRUE, confint_col = 1, confint = "line", ref = FALSE)
```

In the _Q-Q plot_ of the linear model the positive curvature, bending above the line at both ends,
indicates that the PIT residuals are too skew to the left; this is also supported by the _PIT histogram_ of normally scaled PIT residuals:
...
Accordingly, a U-shape pattern can be seen in the _worm plot_:
