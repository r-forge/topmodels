...
Wich actually corresponds to the maximum likelihood estimator for this distriubtion
fitting a generalized linear model (GLM) to the data that links
the expected number of goals per team/match $\lambda_i$ to the linear predictor
$x_i^\top \beta$ with regressor vector $x_i^\top$
...
Here, in the simplest case we fit an intercept-only model where the regressor vector 
consists solely of a constant intercept without further regressors ($x_i^\top = 1$):
...
The corresponding prediction for the
number of goals can be obtained manually from the extracted `coef()`
...
Hence, the regressor vector is $x_i^\top = (1, \mathtt{difference}_i)$
where as for the intercept-only model a log-link is employed:
...
Here, the slope of $`r round(coef(m_prob)[2], digits = 3)`$ can be interpreted
