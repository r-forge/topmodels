...
\Abstract{
  ...
  implementation is introduced and illustrated by numerical weather prediction
...
package \cite{zeileis2010} \code{formula} can have two parts separated by \samp{|} where the first
...
\begin{tabular}{lp{12.3cm}}
  ...
  \hline
  \code{predict()} & Predictions for new data where \code{"type"} controls whether location (\code{"response"}/\code{"location"}), scale (\code{"scale"}) or quantiles (\code{"quantile"}) are predicted. Quantile probabilies are specified by \code{at}. \\
...
for location, scale, and shape. Note that with the add-on packages
