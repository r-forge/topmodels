...
distribution does not dependent on covariates.
...
\CRANpkg{betareg} \citep{zeileis2012} amongs others.
...
Note that \CRANpkg{gamlss} \citep{rigby2005} with the add-on packages
\CRANpkg{gamlss.cens} and \CRANpkg{gamlss.tr} provides a very
...
@
%
Note, that \code{pclogis()} could be replaced by \code{plogis()} since they
...
\CRANpkg{truncreg}. 
%With the possibility to model the dispersion separately,
...
%
\CRANpkg{crch} provides a convenient interface to maximum likelihood estimation of
linear censored and truncated regression models. It supports Gaussian,
logistic or student-t distributed censored or truncated respones 
and provides convenient methods for analysis and prediction
like e.g., \code{predict()}, \code{summary()}, \code{residuals()}, \code{logLik()}.
