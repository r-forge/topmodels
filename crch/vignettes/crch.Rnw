...
\Abstract{
  This paper presents the crch package to fit censored or truncated regression
  models with conditional heteroscedasticity. It provides a convenient interface
  to maximum likelihood and several standard methods for analysis and
  prediction. Censored or truncated Gaussian, logistic, or student-t
  distributions are supported and conditional heteroscedasticity can be
  considered with different regressors for location and scale. This paper
  describes the models and their R implementation and illustrates them with
  numerical weather prediction data of precipitation in Innsbruck (Austria).
}

\Keywords{censored regression, truncated regression, tobit model, Cragg model, heteroskedasticity, \proglang{R}}
...
Data outside this range are also reported but clustered at the range limits. In
contrast, if data outside this range are omitted completely we call it
truncated. E.g., consider that car velocities are measured but the exact values
are only of interest if they exceed a speed limit. If velocities below that
limit are reported as $\le$\textit{speed-limit} the data is censored. If only
velocities exceeding the speed limit are reported and those below are omitted
the data is truncated.

The tobit \citep{tobin1958} and truncated regression \citep{cragg1971} models
are common linear regression models that consider censored and truncated
responses respectively.  Beside truncated data, truncated regression is also
...
homoscedasticity which means that the variance does not dependent on covariates.
...
\citep{simas2010}. There also exist several R packages with functions
implementing the above models, e.g., \CRANpkg{dglm}, \CRANpkg{glmx},
\CRANpkg{gamlss}, \CRANpkg{betareg} amongs others.

The \CRANpkg{crch} package extends censored and truncated regression models to
...
analysis and prediction. Beside the typical conditional Gaussian distribution
assumptions it also allows for heavier tail logistic and student-t
distributions.
...
differentiable function; e.g., the log ($g(\sigma)=\log(\sigma)$) is a well
suited function. Although they also support negative values, the identity
...
log-likelihood function  $l(\beta, \gamma, \nu) = \sum_{i = 1}^n \log(L_i(\mu_i,
\sigma_i))$, where
%
\begin{equation}
  L_i(\mu_i, \sigma_i) =  
...
\end{equation}
%
where the subscript $i=1,\ldots, n$ denotes the events in the data set and $F()$
...
The models described in the previous section can both be fitted with the
\code{crch()} function provided by the \CRANpkg{crch} package.  This function
takes a formula and data, sets up the likelihood function, gradients and Hessian
...
package \code{formula} can have two parts separated by \samp{|} where the first
...
censored or truncated model is estimated. Note that also a convenience function
...
residual standard deviation from \code{lm()}. The remaining coefficients are
initialized with 0. If the degrees of freedom of a student-t distribution are
estimated they are initialized by 10. For the student-t distribution with
...
\pkg{crch} package also provides probability density, cumulative distribution,
...
This section shows an example application of censored and truncated regression
...
Furthermore a two part model tests whether the occurence of precipitation and
its amount are driven by the same process.

First, the \pkg{crch} package is loaded together with a contained precipitation
...
@
%
The \code{data.frame} \code{RainIbk} contains observed 3 days accumulated
precipitation amounts (\code{rain}) and corresponding 11 member ensemble
...
observations. Thus censored regression or a two-part model are suitable models
to estimate this relationship.

First we fit a logistic censored model for \code{rain} with \code{ensmean} as
regressor for the location and \code{enssd} as regressor for the scale.
%
<<>>=
...
@
%
Both, \code{ensmean} and \code{enssd} are highly significant according to the
...
\end{figure}
%
Next we compare the censored model with a two part model consisting of a
heteroscedastic logit model and a truncated regression model with logsitic
...
@
%
In the heteroscedastc logit model the intercept of the scale model is not
identified. Thus, the location coeficients of the censored and truncated
...
occurence of precipitation and its amount.  This is also confirmed by AIC and
BIC that are clearly superiour for the two part than for the censored model:
%
<<>>=
...
statistical applications. However often the homoscedasticity assumtption of
these models is not fullfilled. This paper presented the \CRANpkg{crch} package
that provides functions to fit heteroscedastic censored or truncated response
models. These functions extend \code{tobit()} from \CRANpkg{AER} and
\code{survreg()} from \CRANpkg{survival} or \code{truncreg()} from
\CRANpkg{truncreg}. With the possibility to model the dispersion separately,
\CRANpkg{crch} is also related to \CRANpkg{dglm}, \CRANpkg{glmx} for
heteroscedastic generalized linear models, \CRANpkg{betareg} for beta regression
with a dispersion model, and \CRANpkg{gamlss} to fit generalized additive models
for location, scale, and shape. Note that with the addon packages
\CRANpkg{gamlss.cens} and \CRANpkg{gamlss.tr} \CRANpkg{gamlss} provides a very
powerfull toolbox to also fit crch models -- possibly also with non-linear
smooth effects. However, \CRANpkg{crch} provides a convenient interface for the
special cases of linear censored and truncated regression models with Gaussian,
logistic or student-t distribution. Gradients and Hessian matrix are computed
analytically (except for student-t distribution with estimated degrees of
freedom) and convenient methods like \code{predict}, \code{residuals},
\code{logLik}, $\ldots$ are available.
