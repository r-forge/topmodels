...
could also be fitted by \CRANpkg{gamlss} \citep{rigby2005} with the add-on packages
...
To illustrate the package we applied heteroscedastic censored and truncated models
to predict precipitation in Innsbruck (Austria).
