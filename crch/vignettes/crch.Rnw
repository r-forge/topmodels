...
differentiable function; e.g., the log (\mbox{$g(\sigma)=\log(\sigma)$}) is a well
...
There exist various \proglang{R} packages to fit censored or truncated
regression models. 
Especially \CRANpkg{gamlss} \citep{rigby2005} with the add-on packages
\CRANpkg{gamlss.cens} and \CRANpkg{gamlss.tr} provides a
powerful toolbox to fit heteroscedastic
models -- possibly also with non-linear smooth effects. 

However, in the following we present the \code{crch()} function
from the \CRANpkg{crch} package that provides a convenient interface for the
special cases of linear censored and truncated regression models with Gaussian,
logistic or student-t distribution. 
This function
...
occurrence of precipitation and its amount.  This is also confirmed by AIC and
...
like e.g., \code{predict()}, \code{summary()}, \code{residuals()}, or \code{logLik()}.
