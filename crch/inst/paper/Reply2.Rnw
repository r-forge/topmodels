...
round(se, 3)
@
It can be seen that also in the heteroscedastic Cragg model the 
error correlation is irrelevant and the coefficients are almost the same as in
the Cragg model that ignors sample selection effects.
\end{document}
