...
did try to follow the suggestions, we now do include the full R results that we
...
work, thought, and care and would certainly be well beyond
the scope of our original submission. 
%Of course, one can argue now that we shy away
...
model in addition to the censored model. But we see no reason why 
%for any application of the two-part model 
we should be most concerned about correlations in a potential selection process.
For example, in our case it seems to be more
important to address the heavy tails in the response by adopting a logistic
distribution.
% -- something that is commonly ignored in econometric
%applications of these kinds of models. 
And additionally one could worry
...
rejoinder only). (2) We added a paragraph at the end of the example section
to remind the reader to carefully check the model specifications, including
...
We want to clarify that we did take your suggestion seriously and tried it on our
...
small/non-significant. But, of course, you are right that there are also a lot
of applications where selection effects matter.

Now we do include a comment at the end of the example section 
that encourages readers to consider
...
occur. Moreover, the extension to a bivariate $t$ or logistic approach
...
Given that there is no evidence for this \emph{``likely inconsistency''}
...
cf_cens <- coef(m_cens)
cf_cens <- c(cf_cens[1:2]/exp(cf_cens[3]), cf_cens, NA)
...
Finally, we extend the previous analysis to heteroscedasticity.
Therefore we first estimate the heteroscedastic tobit and Cragg
models like we did in the manuscript but use a normal instead of the
logistic distribution and extract the corresponding coefficients
and standard errors like above.  
<<heteroscedastictobitcragg>>=
m_cens_h <- crch(rain~ensmean | log(enssd), data = RainIbk, left = 0,
  dist = "gaussian")
cf_cens_h <- coef(m_cens_h)
cf_cens_h <- c(cf_cens_h[1:2]/exp(cf_cens_h[3]), cf_cens_h[4], cf_cens_h, NA) 
se_cens_h <- c(NA, NA, NA, sqrt(diag(vcov(m_cens_h))), NA)

library("glmx")
m_bin_h <- hetglm(I(rain > 0) ~ ensmean | log(enssd), data = RainIbk,
         family = binomial(link = "probit"))
m_trunc_h <- crch(rain~ensmean | log(enssd), data = RainIbk, subset = rain > 0, 
         left = 0, truncated = TRUE, dist = "gaussian")
cf_cragg_h <- c(coef(m_bin_h), coef(m_trunc_h), 0)
se_cragg_h <- c(sqrt(diag(vcov(m_bin_h))), sqrt(diag(vcov(m_trunc_h))), NA)
@
Next we build a function for the negative log-likelihood of a heteroscedastic
Cragg model with binomial normal distributed errors:
<<hetcragg>>=
X1 <- X2 <- cbind(1, RainIbk$ensmean)
X3 <- X4 <- cbind(1, log(RainIbk$enssd))
X3 <- X3[, -1, drop = FALSE]
y <- RainIbk$rain
...
nloglik <- function(param)
{
    ...
    ll.pos <- -log(sigma2) + dnorm(resid/sigma2, log = TRUE) +
        pnorm((mu1/sigma1 + rho / sigma2 * resid) / sqrt(1 - rho^2) , log.p = TRUE) -
	pnorm(mu2/sigma2, log.p = TRUE)
    
    -sum(ll.null * (y <= 0) + ll.pos * (y > 0))
}
@
We use the coefficients of the homoscedastic Cragg model as starting values,
optimize via \texttt{optim()}, and extract the coefficients and standard errors:
<<hetcragg2>>=
p <- coef(m_mhd)
p <- c(p[1:4], 0, log(p[5]), 0, p[6])
opt <- optim(p, nloglik, method = "BFGS", hessian = TRUE)

cf_mhd_h <- c(opt$par[1:2], opt$par[5], opt$par[3:4], opt$par[6:8])
se_mhd_h <- sqrt(diag(solve(opt$hessian)))
se_mhd_h <- c(se_mhd_h[1:2], se_mhd_h[5], se_mhd_h[3:4], se_mhd_h[6:8])
@
Now we can compare the estimated coefficients and standard errors
of the 3 different heteroscedastic models:
<<comparison>>=
cf <- cbind(cf_cens_h, cf_cragg_h, cf_mhd_h)
se <- cbind(se_cens_h, se_cragg_h, se_mhd_h)
dimnames(cf) <- dimnames(se) <- list(
  c("S:(Intercept)", "S:ensmean", "S:(scale)_log(enssd)", "O:(Intercept)", 
    "O:ensmean", "O:(scale)_(Intercept)", "O:(scale)_log(enssd)", "rho"),
  c("Tobit", "Cragg", "Cragg (corr)")
)
round(cf, 3)
round(se, 3)
@
It can be seen that, like in the homoscedastic case,
the correlation terms appears to be irrelevant. The coefficients in the Cragg model
do not change significantly and here even the standard errors are very similar
(except maybe for the intercept terms where they are slightly inflated).
