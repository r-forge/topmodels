...
to remind the reader to carefully check the model specifications, e.g., 
...
models like we did in the manuscript (except assuming normality) 
and extract their coefficients and standard errors like above.  
...
optimize via optim(), and extract the coefficients and standard errors:
<<hetcragg2>>=
p <- coef(m_mhd)
...
the error correlation seems irrelevant. The coefficients in the Cragg model
do not change significantly if considering error correlations but the
standard errors seem to be slightly inflated.
\end{document}
