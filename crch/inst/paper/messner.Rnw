...
Altough the above model is apparently well suited for our application
the model specifications should always be carefully considered by
the package user (distribution, scale link functions, regressors, 
non-linearities, etc.).
The two-part model also does not consider sample selection effects
since we found them not to be important for our data.
However, if such effects exist they could be considered, e.g., for the 
homoscedastic and normally distributed case 
with the R-packages \CRANpkg{sampleSelection} \citep{toomet2008} or
\CRANpkg{mhurdle} \citep{croissant2013}. 
