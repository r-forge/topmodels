...
Rain <- RainIbk
Rain$ensmean <- Rain$sqrtensmean
Rain$enssd <- Rain$sqrtenssd
Rain$rain <- sqrt(Rain$rain)
...
\begin{frame}[fragile]
\frametitle{Aalborg forecast (Wed-Fri)}
<<echo=FALSE>>=
load("RainAalb.rda")
...
user <- user[,4:15]
Raintoday <- data.frame(sqrtensmean = mean(sqrt(rowSums(user))), 
  sqrtenssd = sd(sqrt(rowSums(user))))
@
<<>>=
...
legend("topright", bty = "n", xjust = 0, cex = 1.5, legend = 
  c(as.expression(bquote(E(y)~"="~.(expect)~"mm")), "",
  as.expression(bquote(P(y>0)~"="~.(round(plogis(0, location, scale,
...
\begin{itemize}
  \item various applications, e.g., precipitation forecasts
  \item R implementation \pkg{crch}, similar to \code{glm()}
  \item standard methods available
\end{itemize}
\pause
\vspace{1cm}
\textbf{nice weather for UseR 2015!}
\end{frame}
